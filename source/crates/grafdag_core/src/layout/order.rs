//! Layered graph structure and crossing reduction.
use {
    super::{
        LayoutConfig,
        Side,
...
pub enum LKind {
    /// A member placement.
    Real(usize),
    /// Midpoint of an edge spanning ranks.
    Dummy,
    /// Virtual node on the container border for an edge leaving the container.
    Exit {
        inst: usize,
        side: Side,
        to_self: bool,
...
pub struct LNode {
    ...
    /// Center x.
    pub x: f64,
    /// Previous x position if known, for stable initial ordering.
...
pub struct LEdge {
    ...
    pub lower: usize,
    pub weight: f64,
    /// Port x on the upper node (bottom side).
    pub x_upper: f64,
    /// Port x on the lower node (top side).
    pub x_lower: f64,
...
impl Layered {
    ...
    fn positions(&self) -> Vec<usize> {
        let mut pos = vec![0; self.nodes.len()];
        for r in &self.ranks {
    ...
    /// Weighted crossings between rank `r` and `r + 1`.
...
/// Order nodes within ranks to reduce crossings. Stable: only changes that
/// strictly reduce the (weighted) crossing count are kept.
pub fn reduce_crossings(l: &mut Layered, config: &LayoutConfig) {
    // Initial order: previous x, then existing order
    for r in 0 .. l.ranks.len() {
        let mut keyed: Vec<(f64, usize, usize)> = l.ranks[r].iter().enumerate().map(|(i, n)| (l.nodes[*n].prev_x.unwrap_or(f64::MAX), i, *n)).collect();
...
fn barycenter_sort(l: &mut Layered, r: usize, use_upper: bool) {
    ...
    for (i, n) in l.ranks[r].iter().enumerate() {
        ...
        let bary = if edges.is_empty() {
            ...
            // Scale to this rank's index space so unconnected nodes mix sensibly
...
/// Adjacent swaps that strictly reduce crossings, trying low-degree nodes
/// first.
fn transpose(l: &mut Layered) {
    for _ in 0 .. 20 {
