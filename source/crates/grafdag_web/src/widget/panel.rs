...
/// A 0..1 slider bound to a prim.
fn slider(state: &Rc<State>, label: &str, prim: &lunk::HistPrim<f64>) -> El {
    let input = el("input").classes(&["gd_slider"]).attr("type", "range").attr("min", "0").attr("max", "1").attr("step", "0.05");
    ...
    return el("label").classes(&["gd_slider_row"]).push(el("span").classes(&["gd_slider_label"]).text(label)).push(input);
}
