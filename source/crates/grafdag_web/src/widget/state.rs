...
pub struct State {
    ...
    /// Canvas width (coarsely rounded) used as the maximum rank width; changing
    /// it triggers a relayout.
...
impl State {
    ...
    /// Nodes visible in the current layout, in layout order (top-left first).
    pub fn visible_nodes(&self) -> Vec<NodeId> {
        let layout = self.layout.borrow();
        let mut out: Vec<(f64, f64, NodeId)> = layout.nodes.iter().filter(|n| !n.ghost).map(|n| (n.rect.y, n.rect.x, n.id.node.clone())).collect();
