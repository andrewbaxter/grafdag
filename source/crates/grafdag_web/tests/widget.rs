...
fn container_title_fills_box() {
    ...
    for flow in Flow::ALL {
        ...
        assert!(
            (left - (g.rect.w - width) / 2.).abs() < 2.,
            "{:?}: title not centered ({} of {:?})",
            flow,
            left,
            g.rect
        );
        ...
        for child in ["a", "b", "c"] {
            ...
            assert!(
                r.y >= g.rect.y + g.title_height - 0.5,
                "{:?}: {} {:?} in the title of {:?}",
                flow,
                child,
                r,
...
fn keyboard_follow_centers_primary_node() {
    ...
    let sibling = || NodeId(if sel(&h).1.as_deref() == Some("b") {
        "c".into()
    } else {
        "b".into()
    });
...
async fn keyboard_follow_eases() {
    ...
    key("ArrowDown", false);
    let (vw, vh) = s.viewport.get();
    let other = NodeId(if sel(&h).1.as_deref() == Some("b") {
        "c".into()
    } else {
        "b".into()
    });
...
async fn layer_fade_animates_selection_does_not() {
    ...
    let stroke =
        gloo_utils::window().get_computed_style(&path).unwrap().unwrap().get_property_value("stroke").unwrap();
...
fn layers_panel() {
    ...
    let checkbox =
        document()
            .query_selector(".gd_layer_row .gd_checkbox")
            .unwrap()
            .expect("layer checkbox")
    ...
    let input =
        document().query_selector(".gd_layers .gd_input").unwrap().unwrap().dyn_into::<HtmlInputElement>().unwrap();
    input.set_value("New");
    let add =
        document()
            .query_selector(".gd_layers .gd_text_button")
            .unwrap()
            .unwrap()
            .dyn_into::<HtmlElement>()
            .unwrap();
...
fn links_are_hit_testable() {
    ...
    assert_eq!(
        target.get_attribute("class").unwrap_or_default(),
        "gd_edge_hit",
        "got {} {:?}",
        target.tag_name(),
    ...
    assert_eq!(
        target.get_attribute("data-edge"),
        Some("e2".into()),
        "got {} {:?}",
        target.tag_name(),
        target.get_attribute("class")
...
fn node_el(id: &str) -> Element {
    return document()
        .query_selector(&format!(".gd_node_wrap[data-node=\"{}\"]", id))
        .unwrap()
        .expect("node element");
}
...
fn node_moving_into_parent_keeps_element() {
    ...
    let z =
        |id: &str| node_el(id).dyn_into::<HtmlElement>().unwrap().style().get_property_value("z-index").unwrap();
    assert_eq!(z("g"), "0");
    assert_eq!(z("d"), "1");
}
...
fn overlay_buttons() {
    ...
    let sibling =
        document().query_selector(".gd_overlay_button[title^=\"New sibling\"]").unwrap().expect("sibling button");
    let next =
        document().query_selector(".gd_overlay_button[title^=\"New node linked\"]").unwrap().expect("next button");
...
fn search() {
    ...
    let input =
        document()
            .query_selector(".gd_search .gd_input")
            .unwrap()
            .expect("search input")
            .dyn_into::<HtmlInputElement>()
            .unwrap();
...
fn search_results_fill_panel_and_scroll() {
    ...
    assert!(
        results.scroll_height() > results.client_height() + 100,
        "{} vs {}",
        results.scroll_height(),
        results.client_height()
    );
...
fn setup(doc: Document) -> Harness {
    if document().query_selector("#gd_test_style").unwrap().is_none() {
...
fn walk_selection_highlight() {
    ...
    let edge_active =
        |id: &str| document()
            .query_selector(&format!(".gd_edge.gd_active[data-edge=\"{}\"]", id))
            .unwrap()
            .is_some();
    mouse(&node_el("a"), 0);
