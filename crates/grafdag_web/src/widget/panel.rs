//! Side panel: layer list (default), node editor, edge editor, search.
use {
    super::{
        state::{
            Mode,
...
fn input_checked(ev: &web_sys::Event) -> bool {
    return ev.target().and_then(|t| t.dyn_into::<HtmlInputElement>().ok()).map(|i| i.checked()).unwrap_or(false);
}

fn focus(e: &El) {
    if let Ok(h) = e.raw().dyn_into::<HtmlElement>() {
        h.focus().ok();
    }
}
...
pub fn build_panel(pc: &mut ProcessingContext, state: &Rc<State>) -> El {
    ...
    content.ref_own(|c| link!((pc = pc), (mode = state.mode.clone()), (), (c = c.weak(), state = Rc::downgrade(state)) {
        ...
        let body = match mode.get() {
            ...
            Mode::EditEdge(id) => build_edge_editor(pc, &state, &id),
            Mode::Search(target) => build_search(pc, &state, target),
        };
        c.ref_clear();
        c.ref_push(body);
...
fn build_layers(pc: &mut ProcessingContext, state: &Rc<State>) -> El {
    let list = el("div").classes(&["gd_layer_list"]);
    list.ref_own(|l| link!((_pc = pc), (version = state.doc_version.clone()), (), (l = l.weak(), state = Rc::downgrade(state)) {
        let _ = version;
        let l = l.upgrade()?;
        let state = state.upgrade()?;
        let rows = {
            let doc = state.doc.borrow();
            doc.layers.iter().enumerate().map(|(i, layer)| layer_row(&state, layer, doc.selected_layer.as_ref() == Some(&layer.id), i % 8)).collect::<Vec<_>>()
        };
        l.ref_clear();
        if rows.is_empty() {
            l.ref_push(el("div").classes(&["gd_panel_hint"]).text("No layers. Nodes without layers are always shown."));
        }
        l.ref_extend(rows);
    }));
    let new_input = el("input").classes(&["gd_input"]).attr("type", "text").attr("placeholder", "New layer name");
    let add = {
    ...
    return el("div").classes(&["gd_layers"]).extend(vec![
        heading("Layers"),
        list,
        el("div").classes(&["gd_row"]).push(new_input).push(add),
        el("div").classes(&["gd_panel_hint"]).text("Layers color node outlines and links. Check to show a layer, click the name to make it the current layer (new nodes and links go there, and layout prioritizes it)."),
        build_help(),
    ]);
}

fn layer_row(state: &Rc<State>, layer: &Layer, selected: bool, color: usize) -> El {
    let id = layer.id.clone();
    let checkbox = el("input").classes(&["gd_checkbox"]).attr("type", "checkbox").attr("title", "Show layer");
    if layer.active {
    ...
    let name = el("span").classes(&["gd_layer_name"]).text(&layer.name).attr("title", "Make current layer");
    name.ref_on("click", {
        let weak = Rc::downgrade(state);
        let id = id.clone();
        move |_| {
    ...
    let swatch = el("span").classes(&["gd_layer_swatch"]).attr("data-layer-color", &color.to_string());
    let row = el("div").classes(&["gd_layer_row"]).extend(vec![checkbox, swatch, name, delete]);
...
fn build_help() -> El {
    let keys = [
        ("Left click / right click", "select start / end node"),
        ("Arrows", "move selection (Shift: choose end among next/previous)"),
        ("Tab", "next island"),
        ("l / u / r", "link / unlink / reverse"),
        ("n / s", "new next node / new sibling"),
        ("e / E / L", "edit node / start node / link"),
        ("f", "flip start and end"),
        ("Delete", "delete node"),
        ("/ ?", "search start / end"),
        ("z / Z", "undo / redo"),
        ("+ - 0", "zoom, fit"),
    ...
    let rows: Vec<El> = keys.iter().map(|(k, v)| {
        el("div").classes(&["gd_help_row"]).push(el("span").classes(&["gd_help_key"]).text(k)).push(el("span").classes(&["gd_help_desc"]).text(v))
    }).collect();
    return el("div").classes(&["gd_help"]).push(heading("Keys")).extend(rows);
}

// Node editor

fn build_node_editor(pc: &mut ProcessingContext, state: &Rc<State>, id: &NodeId) -> El {
    let textarea = el("textarea").classes(&["gd_textarea"]).attr("rows", "4").attr("placeholder", "Node text");
    textarea.ref_own(|t| link!((_pc = pc), (version = state.doc_version.clone()), (), (t = t.weak(), state = Rc::downgrade(state), id = id.clone()) {
        let _ = version;
        let t = t.upgrade()?;
        let state = state.upgrade()?;
    ...
    *state.focus_request.borrow_mut() = Some(textarea.clone());
    let dynamic = el("div");
    dynamic.ref_own(|d| link!((_pc = pc), (version = state.doc_version.clone()), (), (d = d.weak(), state = Rc::downgrade(state), id = id.clone()) {
        let _ = version;
        let d = d.upgrade()?;
        let state = state.upgrade()?;
        d.ref_clear();
...
fn node_editor_fields(state: &Rc<State>, id: &NodeId) -> Vec<El> {
    let doc = state.doc.borrow();
    ...
    if !doc.layers.is_empty() {
        ...
        for layer in &doc.layers {
            let checkbox = el("input").classes(&["gd_checkbox"]).attr("type", "checkbox");
            ...
            out.push(el("label").classes(&["gd_check_row"]).push(checkbox).push(el("span").text(&layer.name)));
        }
    }

    // Parents
    out.push(el("div").classes(&["gd_field_label"]).text("Parents (drawn inside)"));
    for p in &node.parents {
        ...
        out.push(el("div").classes(&["gd_parent_row"]).push(el("span").classes(&["gd_parent_name"]).text(&node_label(state, p))).push(remove));
    }
    ...
    for other in &doc.nodes {
        ...
        select.ref_push(el("option").attr("value", &other.id.0).text(&node_label(state, &other.id)));
    }
...
fn build_edge_editor(pc: &mut ProcessingContext, state: &Rc<State>, id: &EdgeId) -> El {
    let input = el("input").classes(&["gd_input"]).attr("type", "text").attr("placeholder", "Link text");
    input.ref_own(|t| link!((_pc = pc), (version = state.doc_version.clone()), (), (t = t.weak(), state = Rc::downgrade(state), id = id.clone()) {
        let _ = version;
        let t = t.upgrade()?;
        let state = state.upgrade()?;
...
fn edge_editor_fields(state: &Rc<State>, id: &EdgeId) -> Vec<El> {
    let doc = state.doc.borrow();
    ...
    out.push(
        el("div")
            ...
            .text(&format!("{} \u{2192} {}", node_label(state, &edge.source), node_label(state, &edge.dest))),
    );
