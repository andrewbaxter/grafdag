//! Canvas rendering: nodes (divs), edges (svg paths),
...
fn node_text_el(e: &El) -> Option<El> {
    let raw = e.raw().first_element_child()?;
    return Some(el_from_raw(raw));
}
...
fn measure(node_el: &El, previous: Option<&Measured>) -> (NodeSize, Option<f64>) {
    let text_el = node_text_el(node_el).unwrap();
    remove_style(node_el, "width");
    remove_style(node_el, "height");
...
fn make_node_el(state: &Weak<State>, id: &PlacementId) -> El {
    ...
    let node = el("div").classes(&["gd_node"]).push(text);
    node.ref_on("mousedown", {
...
fn apply_layout(state: &Rc<State>, layout: &Layout, nodes_el: &El, svg_el_: &El, paths_el: &El, labels_el: &El) {
    ...
    for n in &layout.nodes {
        ...
        let color = doc.node(&n.id.node).and_then(|nn| nn.layers.iter().find(|l| doc.layer_active(l))).and_then(|l| layer_color_index(&doc, l));
        match color {
    ...
    for e in &layout.edges {
        ...
        p.ref_modify_classes(&[("gd_edge_reversed", e.reversed)]);
        p.ref_attr("data-edge", &e.id.0);
        if let Some(i) = doc.edge(&e.id).and_then(|edge| edge.layer.as_ref()).and_then(|l| layer_color_index(&doc, l)) {
            p.ref_attr("data-layer-color", &i.to_string());
        }
        render.edge_els.push((p.clone(), e.source.node.clone(), e.dest.node.clone()));
        new_paths.push(p);
        if let Some(edge) = doc.edge(&e.id) {
...
fn apply_selection(state: &Rc<State>) {
    ...
    for (id, e) in &render.node_els {
        e.ref_modify_classes(
            &[("gd_node_start", Some(&id.node) == start.as_ref()), ("gd_node_end", Some(&id.node) == end.as_ref())],
        );
    }
    let focus = end.clone().or(start.clone());
    for (e, s, d) in &render.edge_els {
        let between = match (&start, &end) {
            (Some(a), Some(b)) => (s == a && d == b) || (s == b && d == a),
            _ => false,
        };
        let touches = focus.as_ref().map(|f| s == f || d == f).unwrap_or(false);
        e.ref_modify_classes(&[("gd_edge_between", between), ("gd_edge_focus", touches && !between)]);
    }
}
...
pub fn build_canvas(pc: &mut ProcessingContext, state: &Rc<State>) -> El {
    ...
    canvas.ref_own(|_| link!((pc = pc), (version = state.doc_version.clone()), (layout = state.layout.clone()), (state = Rc::downgrade(state), nodes = nodes.clone()) {
        let _ = version;
        ...
        let new_layout = {
            let doc = state.doc.borrow();
            grafdag_core::layout::layout(&doc, &sizes, &LayoutConfig::default(), Some(&previous))
        };
