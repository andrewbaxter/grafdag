...
pub fn icon_button(state: &Rc<State>, icon: &str, title: &str, cb: impl Fn(&State, &mut ProcessingContext) + 'static) -> El {
    ...
    return el("button")
        ...
        .attr("title", title)
        .attr("type", "button")
        .push(el("span").classes(&["gd_icon", &format!("gd_icon_{}", icon)]))
        .on("click", move |_| {
...
pub fn build_toolbar(pc: &mut ProcessingContext, state: &Rc<State>) -> El {
    ...
    let pair_buttons = vec![
        ...
        icon_button(state, "swap_horiz", "Flip start/end (f)", |s, pc| s.cmd_flip(pc)),
    ];
    ...
    items.push(icon_button(state, "view_sidebar", "Toggle side panel", |s, pc| s.cmd_toggle_panel(pc)));
